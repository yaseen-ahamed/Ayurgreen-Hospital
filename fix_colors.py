...
for f in files:
    ...
    if '.ayur-condition-card {' in content:
        # Extract the specific block for ayur-condition-card
        match = re.search(r'\.ayur-condition-card\s*\{([^}]+)\}', content)
        if match:
            card_block = match.group(1)
            # Find the background color in this specific block
            color_match = re.search(r'background:\s*linear-gradient\(.*?\),\s*(#[A-Fa-f0-9]{3,6})', card_block)
            if color_match:
