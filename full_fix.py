...
for f in files:
    ...
    if '.ayur-condition-card {' in content:
        # First fix the gradient everywhere uniformly
        content = re.sub(r'background:\s*linear-gradient\(.*?\),\s*(#[A-Fa-f0-9]{3,6});',
                         r'background: linear-gradient(90deg, rgba(0,0,0,0.4) 0%, rgba(255,255,255,0.3) 100%), \1;', content)
        
        # Now recalculate colors
        match = re.search(r'background:\s*linear-gradient\(.*?\),\s*(#[A-Fa-f0-9]{3,6})', content)
        if match:
